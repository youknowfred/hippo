...
# Read-only over ledger + index; fires ONLY inside semantic_reverify's demote+
# superseded_by verdict (the existing single-item, AST-pinned path — no replay_all
# verb exists, and none may). Zero hot-path cost: this is verdict-time work.
# --------------------------------------------------------------------------- #
    ...
    *,
    ...
    try:
        ...
        if successor_path is not None:
            ...
            if not dry_run:
                # TMB-5: the succession replay — did the OLD name's recall surface
                # transfer to the successor? Fires only here, inside the single-item
                # demote+superseded_by verdict (no replay_all verb exists); read-only;
                # its summary rides the SAME ledger event below (no new ledger, and the
                # event still carries names + counts only, never query text).
        ...
        result["logged"] = record_reconsolidation_outcome(
            ...
            # TMB-5: counts only — the reconsolidation ledger's no-sensitive-content
            # contract (names + outcomes) holds; preview text stays in the recall ledger.
            succession_replay=(
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.reverify:
        ...
        # TMB-5: the succession replay's per-query PASS/FAIL/INCONCLUSIVE lines — printed
        # at verdict time, right where the demote+supersede was rendered.
