...
Link density (GRA-3), edge rot (GRF-1), salience evidence (MSR-5), hot-path latency
(INT-5, KPI-3), recall channels/blind spots/drop autopsy (MSR-3, SIG-3, MSR-4), injection
precision (SIG-4, KPI-2), the rules-plane trio (RUL-1, RUL-2, RUL-4), and the T11
succession/update instruments.

The dense-floor and abstention checks (GRF-3, RET-9, RET-11) live in
``doctor_checks_abstention`` — they read the floor sweep and the corpus's off-topic
fixture, which nothing else here touches. ``DoctorContext`` lives in ``doctor_checks_env``.
"""
