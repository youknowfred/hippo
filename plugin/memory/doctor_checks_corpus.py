...
Steering (GOV-2), format/derivation versions (COR-7, DRV-2), the VOL-1 volatile-path
arming-policy line, pack drift (TEA-2, RCH-5), FILL-ME templates (ONB-4), the trust
spine (SEC-1, SEC-6), secrets (SEC-2), committed-usage privacy (SEC-14), the
dream-ledger reconcile (DRM-2), and the RET-3 non-English-corpus heuristic with its
Latin-script tables. The lifecycle/collaboration/publish tail lives in
``doctor_checks_lifecycle`` (REL-1); ``DoctorContext`` lives in ``doctor_checks_env``.
"""
    ...
    try:
        ...
        if cite >= CITATION_DERIVATION_VERSION:
            ...
            parts.append(
                ...
                "./; v2 to extensionless files like Dockerfile; v3 to .mdc Cursor rule "
                "sources), so some memories watch the wrong file and some are staleness-"
...
def check_volatile_paths(ctx: DoctorContext) -> Dict[str, str]:
    ...
    belong to doctor, never the hot path. Repatriated here from the ``doctor.py``
    façade by REL-1's split of this file (the cap pressure that exiled it — ED5R-3's
    founding distortion — is discharged; corpus checks live in the corpus-checks home).
    """
    try:
