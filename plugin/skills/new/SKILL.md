...
- **On Claude Desktop** (you have the `⌨ Surface note` in your context, or `CLAUDE_CODE_ENTRYPOINT` is `claude-desktop`): the Desktop path for this verb IS the `new_memory` MCP tool — call it directly to write the memory (pass `check:true` first if you want the near-duplicate/conflict report). Skip the bash preflight and the shell blocks below; those run only in a terminal. Call the tool with no preamble — don't explain that typed commands or the shell flow don't work on this surface, or why you're reaching for a tool instead of bash. That surface-plumbing narration is exactly the repeated noise this routing removes.
